...
fn smooth_samples(n: usize) -> Vec<GlonassSample> {
    (0..n)
        .map(|i| GlonassSample {
            ...
            slot: GloSlot::new(1).unwrap(), // если slot должен быть GloSlot
            cn0_dbhz: DbHz::new(42 + (i % 5) as u8).unwrap(), // исправлено
...
fn multi_slot_samples(n: usize) -> Vec<GlonassSample> {
    ...
    (0..n)
        .map(|i| GlonassSample {
            timestamp_ms: BASE_TS + i as u64,
            slot: GloSlot::new(slots[i % 4]).unwrap(), // slot как GloSlot
            cn0_dbhz: DbHz::new(38 + (i % 8) as u8).unwrap(), // cn0_dbhz как DbHz
