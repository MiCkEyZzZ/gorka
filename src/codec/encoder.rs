...
use crate::{
    encode_i64, BitWriter, FormatVersion, GlonassSample, GorkaError, MilliHz, Millimeter,
    VersionUtils,
};
...
fn encode_verbatim(
    ...
    out.push(sample.slot as u8);
    out.push(sample.cn0_dbhz);
...
fn encode_cn0(
    ...
    let delta = cn0 as i16 - state.last_cn0 as i16; // range -255..=255
...
mod tests {
    ...
    #[test]
    fn test_invalid_slot_in_second_sample_returns_error() {
        let good = sample(0, 1);
        let bad = GlonassSample {
            slot: 7,
            ..sample(1, 1)
        };
        let err = GlonassEncoder::encode_chunk(&[good, bad]).unwrap_err();

        assert!(matches!(err, GorkaError::InvalidSlot(7)));
    }

    #[test]
    fn header_magic_is_correct() {
        let buf = GlonassEncoder::encode_chunk(&[sample(0, 1)]).unwrap();
    ...
    fn test_constant_signal_compresses_well() {
        ...
        let samples: Vec<_> = (0..256).map(|i| constant_sample(i, 1)).collect();
    ...
    fn test_smooth_signal_compresses_well() {
        let samples: Vec<_> = (0..512).map(|i| sample(i, 1)).collect();
        let buf = GlonassEncoder::encode_chunk(&samples).unwrap();
    ...
    fn test_more_samples_give_better_ratio_than_less() {
        let small: Vec<_> = (0..8).map(|i| sample(i, 2)).collect();
        let large: Vec<_> = (0..128).map(|i| sample(i, 2)).collect();
    ...
    fn test_all_valid_slots_encode_without_error() {
        for slot in -7_i8..=6 {
            let samples: Vec<_> = (0..16).map(|i| sample(i, slot)).collect();
    ...
    fn test_multi_slot_chunk_encodes_without_error() {
        ...
        for i in 0..32u64 {
            samples.push(sample(i * 2, 1));
            samples.push(sample(i * 2 + 1, -3));
        }
    ...
    fn test_no_carrier_phase_throughout() {
        let samples: Vec<_> = (0..32).map(|i| constant_sample(i, 0)).collect();
    ...
    fn test_carrier_phase_acquired_mid_stream() {
        let mut samples: Vec<_> = (0..8).map(|i| constant_sample(i, 0)).collect();
    ...
    fn test_carrier_phase_lost_mid_stream() {
        let mut samples: Vec<_> = (0..8).map(|i| sample(i, 0)).collect();
    ...
    fn test_timestamp_dod_zero_emits_one_bit() {
        ...
        let two = (0..2).map(|i| constant_sample(i, 0)).collect::<Vec<_>>();
        let three = (0..3).map(|i| constant_sample(i, 0)).collect::<Vec<_>>();
    ...
    fn test_large_timestamp_gap_uses_verbatim_bucket() {
        ...
        let s0 = constant_sample(0, 0);
        let s1 = constant_sample(1, 0);
